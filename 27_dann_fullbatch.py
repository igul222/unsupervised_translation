...
parser.add_argument('--lambda_custom', type=float, default=0.0)
...
def forward():
    ...
    # Asymmetry Penalty
    # with torch.cuda.amp.autocast(enabled=False):
    #     zs_ = (zs*zs.norm(p=2,dim=2,keepdim=True)).float()
    #     zs_cov = torch.einsum('inx,iny->ixy', zs_, zs_) / float(zs_.shape[1])
    # _, eigenvals, components = torch.svd(zs_cov)
    # zs_pca = torch.bmm(zs, components)
    # eigenval_ratios = eigenvals[:,1:] / eigenvals[:,:-1]
    # custom_penalty = eigenval_ratios.max(dim=1)[0].mean()
    ...
    loss = (
        ...
        + (args.lambda_custom * custom_penalty)
    )

    return loss, erm_loss, disc_loss, grad_penalty, cov_penalty, custom_penalty
...
for step in range(10001):
    ...
    histories['custom_penalty'].append(custom_penalty.item())
    ...
    if step % 100 == 0:
        ...
        utils.print_row(
            ...
            np.mean(histories['custom_penalty']),
            np.min(test_accs),
            np.median(test_accs),
            np.max(test_accs)
        )
