"""
Can we translate MNIST to itself (i.e. recover the identity map among the family
of linear maps) by minimizing an adversarial loss between distributions?

Answer: Yes! It usually takes about 10 random restarts to find a good solution,
but then we succeed. 
"""
...
def init_model():
    generator = nn.Linear(784, 784).cuda()
...
def forward(detach=False):
    x_real = lib.get_batch(mnist_data, BATCH_SIZE)
    x_fake = generator(lib.get_batch(mnist_data, BATCH_SIZE))
...
def l2_eval():
    x_real = lib.get_batch(mnist_data, 4096)
    ...
    for step in range(5001):
        ...
        if step % 1000 == 0:
            ...
            lib.save_image_grid_mnist(mnist_data[:100].cpu().numpy(),
                f'outputs/06_restart{restart}_original.png')
            lib.save_image_grid_mnist(
                generator(mnist_data[:100]).cpu().detach().numpy(),
                f'outputs/06_restart{restart}_translated.png'
            )