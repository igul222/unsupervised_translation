...
T = (lib.ops.random_orthogonal_matrix(2)
    @ torch.tensor([[2., 0.], [0., 1.]], device='cuda')
    @ lib.ops.random_orthogonal_matrix(2))
Xt = Xt @ T.T

plot(Xs, 'Xs')
plot(Xt, 'Xt')

pca_Xs = lib.pca.PCA(Xs, 2, whiten=True)
pca_Xt = lib.pca.PCA(Xt, 2, whiten=True)
...
Xs_feat_pca = lib.pca.PCA(Xs_feat, Xs_feat.shape[1], whiten=True)
Xt_feat_pca = lib.pca.PCA(Xt_feat, Xt_feat.shape[1], whiten=True)
...
for i in range(Xs_feat_white.shape[1]):
    p = Xs_feat_white[:, i]
    q = Xt_feat_white[:, i]
    unflipped_dist = lib.ops.wasserstein_1d(p, q)
    flipped_dist = lib.ops.wasserstein_1d(p, -q)
    if flipped_dist < unflipped_dist:
        p *= -1

D = lib.ops.distance_matrix(Xs_feat_white, Xt_feat_white)
